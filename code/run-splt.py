...
with tf.device('GPU:0'):
    ...
    l_out_1 = get_unet_spleeter(l_input, kernel_size=(6, 4))
    l_out_2 = get_unet_spleeter(l_input, kernel_size=(4, 6))
...
checkpoint_path = "../checkpoints/ws-3-6-splt-{epoch:02d}_2.hdf5"
...
with tf.device('GPU:0'):
    ...
    model.compile(loss=tf.keras.losses.mae, optimizer=Adam(
        lr=8e-3, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0))
    #model.compile(loss=tf.keras.losses.mae, optimizer= SGD(lr=1e-2, decay=1e-7, momentum=0, nesterov=True))

    model.load_weights('../checkpoints/4_6_3globo.hdf5')
