...
with tf.device('GPU:0'):
    ...
    l_out_1 = get_unet_spleeter(l_input, kernel_size=(4, 6))
    l_out_2 = get_unet_spleeter(l_input, kernel_size=(6, 4))
    concat_layer = concatenate([l_out_1, l_out_2])
    mask_layer = Conv2D(1, (1, 1),use_bias=False)(concat_layer)
...
checkpoint_path = "../checkpoints/ws-6-4-splt-{epoch:02d}-{val_loss:.3f}_2.hdf5"
...
with tf.device('GPU:0'):
    ...
    model.compile(loss=tf.keras.losses.mae, optimizer=Adam(
        lr=5e-3, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0))
    #model.compile(loss=tf.keras.losses.mae, optimizer= SGD(lr=1e-2, decay=1e-7, momentum=0, nesterov=True))

    #model.load_weights('../checkpoints/ws-3-7-splt-55-0.079_2.hdf5')
